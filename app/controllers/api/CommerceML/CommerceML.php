...
use Zenwalker\CommerceML\Model\Currency;
use Zenwalker\CommerceML\Model\CurrencyCollection;
...
class CommerceML
{
     ...
     * @return void
     */
    public function parseCurrency($offersXml)
    {
        if ($offersXml->ПакетПредложений->ТипыЦен) {
            foreach ($offersXml->ПакетПредложений->КурсыВалют->Валюта as $xmlCurrency) {
                $currency = new Currency($xmlCurrency);
                $this->getCollection('currency')->add($currency);
            }
        }
    }
