<link rel="stylesheet" type="text/css"
    href="<?=SERVER_URL . 'style/' . $_SESSION['alias']->alias . '/shop.css'?>">
...
<main class="container product" itemscope itemtype="https://schema.org/Product">
    ...
    <a href="<?=SERVER_URL?>admin/<?=$product->link?>"
        class="pull-right btn btn-warning" target="_blank"><i class="fas fa-cogs"></i> Редагувати Admin</a>
    <?php }
    if ($product->author_add == $_SESSION['user']->id || $this->userCan()) { ?>
    <a href="<?=SITE_URL . $product->link?>?edit"
        class="pull-right btn btn-success" target="_blank"><i class="fas fa-paint-brush"></i> Редагувати</a>
    <?php }
}
    ...
    <div class="alert alert-warning">
        ...
        <p>Коли вказано всю інформацію про товар, сторінка виглядає гармонійно, натисніть кнопку "Надіслати товар на
            перевірку". <br>Адміністрація сайту перевірить вказану інформацію та активує товар або ж надішле
            повідомлення щодо відмови вказавжи причину.</p>
        <form
            action="<?=SITE_URL . $_SESSION['alias']->alias?>/confirm"
            method="post">
    ...
    <div class="alert alert-success">
        ...
        <p>Адміністрація сайту перевірить вказану інформацію та активує товар або ж надішле повідомлення щодо відмови
            вказавжи причину.</p>
    </div>
    <?php }
    ...
    <div class="d-flex m-wrap">
        ...
        foreach ($_SESSION['alias']->images as $i => $image) {
            ...
            <figure data-thumb="<?=IMG_PATH . $image->thumb_path?>"
                ...
                <figcaption><?=$image->title ?? $product->name?>
                </figcaption>
            </figure>
        ...
        <div class="w33-5 m-w100 specifications">
                ...
                <tbody>
                    ...
                    if (!empty($product->options)) {
                        foreach ($product->options as $option) {
                            ...
                            } elseif (!empty($option->value) && is_array($option->value)) {
                                $values = [];
                    ...
                    <tr>
                        ...
                        <td><?=date('d.m.Y H:i', $product->date_add)?>
                        </td>
                    </tr>
                </tbody>
            </table>
            ...
            <?php }
            if (!empty($author->info['shipping'])) {
                $shipping = str_replace('-', '<li>', $author->info['shipping']); ?>
            <div class="shipping-info bordered">
                <h4>Доставка</h4>
                <ul><?=html_entity_decode($shipping)?>
                </ul>
            </div>
            <?php
            } ?>
            <div class="shipping-info bordered" itemprop="aggregateRating" itemscope
                itemtype="https://schema.org/AggregateRating">
                <h4>Оцінка <?php if (empty($product->rating)) {
                $product->rating = 0;
            }
        ...
        <div class="w33-5 m-w100">
            <div class="author bordered d-flex">
                ...
                <a href="/seller/<?=$author->alias?>"><img
                        src="<?=$user_photo?>"
                        alt="<?=$author->name?>"></a>
                <div>
                    ...
                    <p>З нами від <i><?=date('d.m.Y', $author->registered)?></i>
                    </p>
                </div>
                <?php
                } else {
                    echo 'Відсутня інформація про автора!';
                } ?>
            </div>
            <div class="order bordered">
                ...
                <a href="<?=SITE_URL . $product->link?>?edit"
                    id="showContacts" class="text-center"><i class="fas fa-cogs"></i> Редагувати</a>
                <?php } else { ?>
                <button id="showContacts"
                    data-id="<?=$product->id?>">Показати контактні
                    дані</button>
                <!-- <p class="or text-center">або</p>
                    <button id="toOrderModel">Залишити заявку</button> -->
                <?php } ?>
            </div>
            <?php if (!empty($author->info['pay'])) {
                    $pay = str_replace('-', '<li>', $author->info['pay']); ?>
            <div class="pay-info bordered">
                <h4>Оплата</h4>
                <ul><?=html_entity_decode($pay)?>
                </ul>
            </div>
            <?php
                } ?>
    ...
    <div id="tabs">
        <ul
            class="tabs <?=(!empty($_SESSION['alias']->videos)) ? '' : 'tabs-4'?>">
            <?php if (!empty($_SESSION['alias']->videos)) { ?>
            <li><a href="#video"><span><?=$this->text('Відео')?></span></a>
            </li>
            <?php } ?>
            <li><a href="#similar"><span><?=$this->text('Подібні')?></span></a>
            </li>
            <li><a href="#more"><span><?=$this->text('Ще товари від') . ' ' . $from_sex?></span></a>
            </li>
            <li><a href="#delivery-and-payments"><span><?=$this->text('Доставка та оплата')?></span></a>
            </li>
            <li><a href="#reviews"><span><?=$this->text('Відгуки')?></span></a>
            </li>
        </ul>
        <?php if (!empty($_SESSION['alias']->videos)) { ?>
        <div id="video">
        ...
        <div id="similar">
            ...
            <div class='p-15 text-center'><a
                    href="<?=SITE_URL . $product->group_link?>"
                    class="btn btn-info">Дивитися більше</a></div>
            <?php }
            } else { ?>
            <p>На даний момент це єдиний товар у групі <strong><?=$product->group_name?></strong></p>
            <?php } ?>
        </div>
        <div id="more">
            ...
            <div class='p-15 text-center'><a
                    ...
                    class="btn btn-info">Дивитися всі товари <?=$from_sex?></a></div>
            <?php }
            } else { ?>
            <p>На даний момент це єдиний товар <?=$from_sex?>
            </p>
            <?php } ?>
        </div>
        <div id="delivery-and-payments">
            <?php // echo $this->function_in_alias('delivery-and-payments', '__get_Text');?>
            <?php if (!empty($author->info['shipping'])) {
                $shipping = str_replace('-', '<li>', $author->info['shipping']); ?>
            <div class="shipping-info bordered">
                <h4>Доставка</h4>
                <ul><?=html_entity_decode($shipping)?>
