...
{
    public $multigroup_new_position = [];
    ...
    public function getProducts($Group = 0, $active = true)
    {
        ...
        if ($_SESSION['option']->useAvailability == 0) {
            ...
            if ($_SESSION['language']) {
                $where_availability_name['language'] = $_SESSION['language'];
            }
    ...
    {
        $this->db->select($this->table() . ' as p', '*', ['wl_alias' => $_SESSION['alias']->id, 'id' => $id]);
            ...
            if ($_SESSION['option']->useGroups) {
                if ($_SESSION['option']->ProductMultiGroup && !empty($_POST['product_groups'])) {
                    ...
                    foreach ($product_groups as $group) {
                        ...
                        $this->db->insertRow($this->table('_product_group'), ['product' => $id, 'group' => $group, 'position' => $all, 'active' => 1]);
                    }
                } else {
                    if (isset($_POST['group']) && is_numeric($_POST['group'])) {
                        $data['group'] = $_POST['group'];
                        $data['position'] = $this->db->getCount($this->table('_products'), ['wl_alias' => $_SESSION['alias']->id, 'group' => $data['group']]) + 1;
    ...
    {
        $data = ['author_edit' => $_SESSION['user']->id, 'date_edit' => time()];
        ...
        if ($_SESSION['option']->ProductUseArticle) {
            ...
            if ($data['article'] != $this->data->post('article_old')) {
                if ($names = $this->db->getAllDataByFieldInArray('wl_ntkd', ['alias' => $_SESSION['alias']->id, 'content' => $id])) {
                    foreach ($names as $row) {
                        ...
                        if ($article_old == $this->data->post('article_old')) {
                            $name .= ' ' . $data['article'];
                            $this->db->updateRow('wl_ntkd', ['name' => $name], $row->id);
                        }
                    }
                }
            }
        ...
        if ($_SESSION['option']->useGroups) {
            if ($_SESSION['option']->ProductMultiGroup) {
                    ...
                    foreach ($product_groups_new as $group) {
                        if (!in_array($group, $activegroups)) {
                            ...
                            if (isset($activegroups_position[$group]) && $activegroups_position[$group]->active != $active) {
                                $this->db->updateRow($this->table('_product_group'), ['active' => $active], $activegroups_position[$group]->id);
                            }
                        }
                        $use[] = $group;
                    }
                ...
                if ($_POST['group'] != $_POST['group_old']) {
                    ...
                    $data['group'] = $_POST['group'];
                    $data['position'] = 1 + $this->db->getCount($this->table('_products'), ['wl_alias' => $_SESSION['alias']->id, 'group' => $data['group']]);
                ...
                if (is_array($value)) {
                    foreach ($value as $lang => $value2) {
                        if ($_SESSION['language'] && isset($list[$key][$lang])) {
                            if ($list[$key][$lang]->value != $value2) {
                                $this->db->updateRow($this->table('_product_options'), ['value' => $value2], $list[$key][$lang]->id);
                            }
                            unset($list[$key]);
                        } elseif (isset($list[$key]) && is_object($list[$key])) {
                            if ($list[$key]->value != $value2) {
                                $this->db->updateRow($this->table('_product_options'), ['value' => $value2], $list[$key]->id);
                            }
                            unset($list[$key]);
                        } else {
                            $data['product'] = $id;
                    ...
                    if (isset($list[$key]) && is_object($list[$key])) {
                        if ($list[$key]->value != $value) {
                            $this->db->updateRow($this->table('_product_options'), ['value' => $value], $list[$key]->id);
                        }
                    } else {
                        $data['product'] = $id;
                        $data['option'] = $key;
    ...
    public function saveChangePrice($id)
    {
        ...
        if (!empty($options)) {
            foreach ($options as $key => $value) {
                ...
                if ($option = $this->db->getAllDataById($this->table('_product_options'), ['product' => $id, 'option' => $key])) {
                    if ($option->changePrice != $changePrice) {
                        $this->db->updateRow($this->table('_product_options'), ['changePrice' => $changePrice], $option->id);
                    }
                } else {
                    $this->db->insertRow($this->table('_product_options'), ['product' => $id, 'option' => $key, 'changePrice' => $changePrice]);
                }
            }
        }
    }
    ...
    private function makeLink($all, $parent, $link)
    {
        ...
        if ($all[$parent]->parent > 0) {
            $link = $this->makeLink($all, $all[$parent]->parent, $link);
        }
        return $link;
    }
    ...
    {
        $Group = $this->db->getAllDataById($this->table(), ['wl_alias' => $_SESSION['alias']->id, 'alias' => $link]);
        $end = 0;
        $link2 = $link;
        while ($Group) {
            $end++;
            $link2 = $link . '-' . $end;
            $Group = $this->db->getAllDataById($this->table(), ['wl_alias' => $_SESSION['alias']->id, 'alias' => $link2]);
        }
        return $link2;
    }
