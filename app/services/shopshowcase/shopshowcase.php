...
class shopshowcase extends Controller
{
    ...
    public function index($uri)
    {
        ...
        if (count($this->data->url()) > 1) {
            ...
            if ($type == 'product' && $product) {
                ...
                if (isset($_GET['edit']) && $_SESSION['option']->userCanAdd) {
                    if ($this->userIs()) {
                        if ($product->author_add == $_SESSION['user']->id || $this->userCan()) {
                            ...
                            $this->load->page_view('manager/edit_view', ['product' => $product, 'groups' => $groups, 'options' => $options]);
                        } else {
                ...
                $this->load->page_view('detal_view', ['product' => $product]);
                ...
                $this->load->page_view('group_view', ['group' => $group, 'subgroups' => $subgroups, 'use_filter' => $use_filter, 'products' => $products, 'filters' => $filters, 'filter_minMaxPrices' => $filter_minMaxPrices, 'catalogAllGroups' => $this->shop_model->getGroups(-1)]);
            } else {//if($this->userIs())
                $this->load->page_404(false);
            }
            // else
            ...
            if ($_SESSION['option']->useGroups) {
                if ($groups = $this->shop_model->getGroups(-1)) {
                    $this->load->page_view('index_view', ['catalogAllGroups' => $groups]);
                } else {
                    ...
                    $this->load->page_view('group_view', ['products' => $products, 'use_filter' => true, 'filters' => $filters, 'filter_minMaxPrices' => $filter_minMaxPrices]);
                }
            } else {
    ...
    public function search()
    {
        ...
        if ($id = $this->data->get('id')) {
            ...
            if (!empty($_GET['name']) && $_SESSION['option']->ProductUseArticle) {
                ...
                if ($products = $this->shop_model->getProducts('%' . $this->makeArticle($name))) {
                    ...
                    $this->setProductsPrice($products);
                    $this->load->page_view('search_view', ['products' => $products]);
                    exit;
                }
            ...
            if (is_numeric($_GET['group']) && $_GET['group'] > 0) {
                ...
                if (!empty($groups)) {
                    ...
                    $this->load->page_view('group_view', ['products' => $products, 'use_filter' => true, 'filters' => $this->shop_model->getOptionsToGroup(), 'catalogAllGroups' => $this->shop_model->getGroups(-1)]);
                } else {
        ...
        } elseif (!empty($_GET['article']) && $_SESSION['option']->ProductUseArticle) {
            ...
            if ($author_add) {
                if ($author_add->status == 1 && $author_add->type <= 3) {
                    ...
                    $_SESSION['alias']->name = $_SESSION['alias']->title = 'Товари від ' . $author_add->name;
            ...
            if ($author_add === false) {
                $this->load->page_view('group_view', ['products' => false, 'use_filter' => false, 'filters' => false, 'catalogAllGroups' => $this->shop_model->getGroups(-1)]);
                exit;
            }
    ...
    public function ajaxGetProducts()
    {
        ...
        $this->setProductsPrice($products);
        $this->load->json(['products' => $products, 'page' => $_GET['page'] + 1, 'group' => $group]);
    }
    ...
    public function ajaxUpdateProductPrice()
    {
        if ($product_id = $this->data->post('product')) {
            if (!empty($_POST['options']) && is_array($_POST['options'])) {
                ...
                $this->load->json(['price' => $price, 'product' => $product_id]);
            }
        }
    }
    ...
    public function ajaxGetGroups()
    {
        ...
        $this->load->smodel('shop_model');
        $this->load->json(['groups' => $this->shop_model->getGroups($parent_id, false)]);
    }
    ...
    public function my()
    {
        if ($this->userIs()) {
            ...
            $this->load->profile_view('manager/index_view', ['groups' => $groups, 'products' => $products]);
        } else {
    ...
    public function add()
    {
        ...
        if ($this->userIs()) {
            ...
            $this->load->profile_view('manager/add_view', ['groups' => $groups, 'options' => $options]);
        } else {
                            ...
                            if ($product = $this->db->getAllDataById($_SESSION['service']->table . '_products', $image->content)) {
                                if ($product->author_add == $_SESSION['user']->id || $this->userCan()) {
                                    ...
                                    if ($_SESSION['language']) {
                                        $this->db->deleteRow('wl_media_text', ['type' => 'photo', 'content' => $image->id]);
                                    }
                        ...
                        if ($video->alias == $_SESSION['alias']->id) {
                            if ($product = $this->db->getAllDataById($_SESSION['service']->table . '_products', $video->content)) {
                                if ($product->author_add == $_SESSION['user']->id || $this->userCan()) {
                                    ...
                                    if ($_SESSION['language']) {
                                        $this->db->deleteRow('wl_media_text', ['type' => 'video', 'content' => $video->id]);
                                    }
    ...
    public function __get_Products($data = [])
    {
        ...
        if (isset($data['article']) && $data['article'] != '') {
            ...
            $group = '%' . $data['article'];
        } elseif (isset($data['group']) && (is_numeric($data['group']) || is_array($data['group']))) {
            $group = $data['group'];
        }
        if (isset($data['limit']) && is_numeric($data['limit'])) {
            $_SESSION['option']->paginator_per_page = $data['limit'];
        }
        if (isset($data['sort']) && $data['sort'] != '') {
            $_SESSION['option']->productOrder = $data['sort'];
        }
        if (isset($data['sale']) && $data['sale'] == 1) {
            $_GET['sale'] = 1;
        }
        if (isset($data['availability'])) {
            $_GET['availability'] = $data['availability'];
        }
        if (isset($data['noInclude']) && $data['noInclude'] > 0) {
            $noInclude = $data['noInclude'];
        }
        if (isset($data['active']) && $data['active'] == false) {
            $active = $data['active'];
        }
        if (isset($data['getProductOptions']) && $data['getProductOptions'] == true) {
